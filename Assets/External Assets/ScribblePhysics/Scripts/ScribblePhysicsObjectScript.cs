...
public class ScribblePhysicsObjectScript : MonoBehaviour 
{
	...
	//
	public void SetTouchId(int inputVar)						{touchId = inputVar;}
	public void SetTextureMaterial(Material inputVar)			{textureMaterial = inputVar;}
	public void SetCameraObject(Camera inputVar)				{cam = inputVar;}
	public void SetDrawDistance(float inputVar)					{drawDistance = inputVar;}
	public void SetLineWidth(float inputVar)					{lineWidth = inputVar;}
	public void SetLineDepth(float inputVar)					{lineDepth = inputVar;}
	public void SetMaterialDensity(float inputVar)				{density = inputVar;}
	public void SetScribbleIsDynamic(bool inputVar)				{scribbleIsDynamic = inputVar;}
	public void SetSmoothScribble(bool inputVar)				{smoothScribble = inputVar;}
	public void SetIntersectionAllowed(bool inputVar) 			{intersectionAllowed = inputVar;}
	public void SetPhysicsMaterial(PhysicMaterial inputVar)		{physicsMaterial = inputVar;}
	public void SetMaxAngleBetweenBoxColliders(float inputVar)	{maxAngleBetweenBoxColliders = inputVar;}
	public void SetSurfacePenetrationFactor(float inputVar)		{surfacePenetrationFactor = inputVar;}
	//
	//***************************************************************//
	...
	void Update () 
	{
		if (!scribbleIsFinished) 
		{
		    ...
		    if (touchId == -1 && Input.GetMouseButton(0)) 
			{
				stopDrawing = false;
				currentPosition = cam.ScreenToWorldPoint(Input.mousePosition + new Vector3(0f,0f,drawDistance));
			...
			if (touchId != -1) 
			{
			    ...
			    foreach (Touch touch in Input.touches) 
				{
					if (touchId == touch.fingerId && (touch.phase != TouchPhase.Ended || touch.phase != TouchPhase.Canceled)) 
					{
		        		stopDrawing = false;
			...
			if (stopDrawing) 
			{
				...
				{ // too small to be used as a scribble, therefore destroying the entire object
				    Destroy(gameObject);
				} else {
					if (smoothScribble) 
					{
	...
	void AttachCapsuleColliders(List<Vector3> pointList) 
	{
		...
		Vector3 dirA = Vector3.zero;;	// direction of segment 'A'
		Vector3 dirB = Vector3.zero;;	// direction of segment 'B'
